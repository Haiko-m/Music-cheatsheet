...
function makeAudioContext() {
  ...
  if (!K) return null;
  try { return new K({ latencyHint: "interactive" }); }
  catch (_) { return new K(); }
}

async function unlockAudio() {
  if (!AC || AC.state === "closed") AC = makeAudioContext();
  if (!AC) return null;

  if (AC.state !== "running") {
    try { await AC.resume(); } catch (_) { /* Safari may require the next gesture */ }
  }

  /* iOS Safari sometimes reports "running" before its output path is unlocked.
     Playing a one-sample silent buffer during the user gesture reliably opens it. */
  if (AC.state === "running" && !AC.__theoryDeskUnlocked) {
    try {
      ...
      source.start(0);
      AC.__theoryDeskUnlocked = true;
    } catch (_) { /* harmless on browsers that do not need this */ }
  }
...
function ctx() {
  if (!AUDIO_READY) {
    AUDIO_READY = unlockAudio().finally(() => { AUDIO_READY = null; });
  }
  return AUDIO_READY;
}

/* Unlock on the earliest real user gesture and again after Safari restores the page. */
if (typeof window !== "undefined") {
  ...
  window.addEventListener("pageshow", prime, { passive: true });
...
async function tone(midi, dur = 0.9, when = 0, vol = 0.18) {
  const c = await ctx();
  if (!c) return;
  const t = c.currentTime + Math.max(0.01, when);
...
const strum = (midis, dur = 1.1, when = 0, vol = 0.15) => midis.forEach((m, i) => { void tone(m, dur, when + i * 0.012, vol); });
async function sine(f, dur, when, vol = 0.13) {
  const c = await ctx(); if (!c) return;
  const t = c.currentTime + Math.max(0.01, when);
  const g = c.createGain(); g.connect(c.destination);
  g.gain.setValueAtTime(0.0001, t);
  g.gain.exponentialRampToValueAtTime(Math.max(0.00012, vol * VOL), t + 0.02);
  g.gain.exponentialRampToValueAtTime(0.0001, t + dur);
