...
const reportAudioStatus = (message) => {
...
function scheduleTone(c, midi, dur = 0.9, when = 0, vol = 0.18) {
  try {
    ...
    reportAudioStatus("Tone scheduling failed");
    return false;
  }
}
        ...
        {whites.map((k, i) => {
          ...
          return (
            <button
              ...
              style={{
                backgroundColor: m ? m.color : "var(--key-white)",
          ...
          return (
            <button
              ...
              style={{
                ...
                backgroundColor: m ? m.color : "var(--key-black)",
                WebkitAppearance: "none", appearance: "none",
...
export default function App() {
  ...
  const [silentHintDismissed, setSilentHintDismissed] = useState(false);

  useEffect(() => {
    AUDIO_STATUS_LISTENER = setAudioStatus;
    if (AC) setAudioStatus("Audio: " + AC.state);

    /* Any tap anywhere unlocks, so most people never see the button.
       Capture phase, so it runs before the component's own handler. */
    const kick = () => { if (!AC || AC.state !== "running") unlockAudio(); };
    window.addEventListener("pointerdown", kick, true);
    window.addEventListener("touchend", kick, true);

    /* iOS parks the context in "interrupted" after a call or a lock screen
       and never recovers by itself. */
    const restore = () => { if (document.visibilityState === "visible" && AC) unlockAudio(); };
  ...
  /* Synchronous: everything must happen inside the tap for iOS to unlock. */
  const enableSound = () => {
    const c = unlockAudio();
    if (!c) { setAudioStatus("Web Audio is not supported"); return; }
    scheduleTone(c, 69, 0.18, 0, 0.14);
    setAudioStatus("Audio: " + c.state);
  };
      ...
      <div data-theme={theme} className="min-h-screen w-full" style={{ background: "var(--bg)", color: "var(--text)", fontFamily: SANS }}>
        ...
        <header className="sticky top-0 z-30" style={{ background: "var(--bg)", borderBottom: "1px solid var(--line)" }}>
          <div className="mx-auto max-w-3xl px-3 pt-2">
            ...
            {IS_IOS && audioStatus === "Audio: running" && !silentHintDismissed && (
              <div className="mt-1 flex items-start justify-end gap-2 text-right" style={{ fontSize: 10, color: "var(--muted)" }}>
                <span style={{ maxWidth: 320 }}>
                  Still silent? Check the Ring/Silent switch on the side of the phone — on iOS it mutes
                  web audio even when the volume is up.
                </span>
                <button type="button" onClick={() => setSilentHintDismissed(true)} aria-label="Dismiss tip"
                  style={{ color: "var(--muted)", fontSize: 12, lineHeight: 1 }}>×</button>
              </div>
            )}
