...
function getAudioContext() {
  ...
  if (!AC || AC.state === "closed") {
    // Do not pass constructor options: some Android WebViews reject them.
...
async function ensureAudio() {
  ...
  if (!AUDIO_STARTING) {
    AUDIO_STARTING = (async () => {
      try {
        if (c.state !== "running") await c.resume();

        // A silent buffer helps iOS route Web Audio to the speaker.
        if (!AUDIO_PRIMED) {
...
function scheduleTone(c, midi, dur = 0.9, when = 0, vol = 0.18) {
  const t = c.currentTime + 0.03 + Math.max(0, when);
