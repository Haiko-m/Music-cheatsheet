...
/*
 * iOS Safari keeps Web Audio suspended until resume() is called from a
 * real user gesture. Crucially, resume() is asynchronous, so scheduling
 * oscillators immediately after calling it can produce silence.
 */
async function ensureAudio() {
  ...
  if (!AUDIO_READY) {
    AUDIO_READY = (async () => {
      try {
        ...
        // Prime Safari's output route with a silent, one-frame buffer.
        ...
        console.warn("Audio could not be started:", err);
        AUDIO_READY = null;
        return null;
      }
    })();
  ...
  const ready = await AUDIO_READY;
  if (ready && ready.state !== "running") {
    AUDIO_READY = null;
    return ensureAudio();
  }
  return ready;
...
function scheduleTone(c, midi, dur = 0.9, when = 0, vol = 0.18) {
  const t = c.currentTime + 0.025 + Math.max(0, when);
...
async function tone(midi, dur = 0.9, when = 0, vol = 0.18) {
  ...
  scheduleTone(c, midi, dur, when, vol);
  return true;
}
...
async function strum(midis, dur = 1.1, when = 0, vol = 0.15) {
  const c = await ensureAudio();
  if (!c || c.state !== "running") return false;
  midis.forEach((m, i) => scheduleTone(c, m, dur, when + i * 0.012, vol));
  return true;
}
...
export default function App() {
  /*
   * Unlock Web Audio on the first genuine touch/click. This also restores
   * audio after Safari has backgrounded the tab or interrupted the context.
   */
  useEffect(() => {
    const unlock = () => { void ensureAudio(); };
    const restore = () => {
      if (document.visibilityState === "visible") void ensureAudio();
    };
